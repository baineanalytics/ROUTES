...
##For editing boundaries##
#  bound <- merge(transitgraph, world_map, by.x = "country", by.y = "region")
#  lat <- c(min(bound$lat), max(bound$lat + 15))
#  long <- c(min(bound$long - 15), max(bound$long + 15))
...
png(file = "b_heat600.png", units="in", width=11, height=6, res=600)
...
gg <- gg + geom_map(map = world_map, aes(map_id = country, fill = sum), color = "gray20", lwd = .05) + 
  scale_fill_distiller(palette = "Blues", name="Birds", na.value = "gray87", direction = 1) + 
...
png(file = "b_trans.png", units="in", width=11, height=8.5, res=300)
